...
| the domain crossings | ⚠️ **measured for the first time, and both figures are new ratchets.** Every geometric instrument here filtered to the audio nets — `parallel_runs()` compares audio against audio, `return_loops()` and `audio_off_its_own_plane()` filter to `AUDIO_FAMILIES`, and `check_crossings()` reads the *netlist*, so it knows which thirteen signals cross the domain boundary and nothing about where. So **MCLK runs 0.428 mm from `SIN6` for 30.2 mm** and **the ADC's bundle crosses the ground split 75.6 mm from `R902`**, the only conductor joining the two planes, and no number in this repo moved for either. `constraints.digital_audio_runs()` and `crossing_returns()` are the instruments; `DIGITAL_AUDIO_MM` = 244.2 and `CROSSING_RETURN_MM` = 75.6. The coupling itself is ~−104 dB and comfortable — the finding is that nothing computed it |
| the silkscreen | ✅ **written, and the board had none at all** — four `Edge.Cuts` lines, **zero** `gr_text`, and 250 of 296 references on `F.Fab`. **Nothing here would have noticed**: a silkscreen is not in a netlist, DRC sees it only when it collides, and `gen_fab.package_layers()` drops a layer that draws *nothing* — 29 references were enough to keep `F.SilkS` non-empty. **A layer that is not blank is not a layer that says anything.** Now: a title block, a connector legend *generated from the netlist*, a name beside every connector, and **274 designators** — 245 at 1.0 mm, 25 shrunk, six named as unplaceable and held by `verify.SILK_UNLABELLED`. The passives were on `F.Fab` for a reason that had expired: it was true of a designator at KiCad's *fixed offset*, and all 250 fit once the position is probed. **And `U9`/`U10`'s designators were printed under the chips** — `REFERENCE_MOVES` nudged them 8 mm off a neighbour into the middle of the package they name; `verify.silk_reference_faults()` is the instrument. DRC 0, closest silk-to-pad 1.00 mm |
| board fixings | ✅ **six, derived and fitted — and `gen_fab.py` writes a package again**, 1186 drill hits of which six are the 3.2 mm unplated tool. **The pattern was first called underivable and that was a claim about a dependency direction**: the enclosure is bespoke and drawn *after* the board, so the fixings are an input to it. `placement.mounting_deflection()` is the arithmetic — **four fixings are worth nothing over two** on a 233 mm board, and six is **16×** stiffer than four where eight is only 5.1× more. Positions are *walked* along each edge until the keep-out clears all 290 courtyards. `mounts.py` places them through `pcbnew` with a rule area each, because an M3 screw through an unvoided hole bridges MAGND to MDGND at every fixing — **six ground stars instead of one**; measured, every fixing now sits in a void with 1.85–3.28 mm to the nearest plane copper. **Nothing had ever reserved the margin**, so `PIN5` ran 183 mm through two of the six and `krt.keepout_rects()` has seven entries now instead of one. Still open, and a decision rather than a defect: the **SE corner is 57 mm from its nearest fixing and carries the USB socket** |
